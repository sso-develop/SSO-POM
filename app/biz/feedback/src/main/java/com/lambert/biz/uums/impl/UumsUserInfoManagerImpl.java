...
public class UumsUserInfoManagerImpl implements UumsUserInfoManager{
	...
	public Result<List<UumsUserInfoModel>> queryUumsUserInfo() {
		try {
			...
			return new Result<List<UumsUserInfoModel>>(models);
		} catch (Exception ex) {
	...
	public Result<Boolean> insertUumsUserInfo(UumsUserInfoModel userInfoModel) {
		try {
			if(StringUtils.isBlank(userInfoModel.getOperatorName())){
				...
				return new Result(ResultCode.ILLEGAL_PARAMS, "operatorName 不能为空 , operatorName="+userInfoModel.getOperatorName());
			}
			if(StringUtils.isBlank(userInfoModel.getRealName())){
				...
				return new Result(ResultCode.ILLEGAL_PARAMS, "realName 不能为空 , realName="+userInfoModel.getRealName());
			}
			if(StringUtils.isBlank(userInfoModel.getMobile())){
				...
				return new Result(ResultCode.ILLEGAL_PARAMS, "mobile 不能为空 , realName="+userInfoModel.getMobile());
			}
	...
	public Result<UumsUserInfoModel> queryUumsUserInfoCountByOperatorName(String operatorName) {
		try {
			...
			if(uumsUserInfoDO == null){
				return new Result(ResultCode.DUPLICATE_DATA,"【"+operatorName+"】用户不存在");
			}
			UumsUserInfoModel model = new UumsUserInfoModel(uumsUserInfoDO.getId(),
					DateUtil.getWebDateString(uumsUserInfoDO.getCreateDate()), 
					DateUtil.getWebDateString(uumsUserInfoDO.getModifyDate()),
			...
			return new Result<UumsUserInfoModel>(model);
		} catch (Exception ex) {
			LOGGER.error(ex.getMessage(), ex);
			return new Result(ResultCode.SERVER_EXCEPTION, ex.getMessage());
		}
	...
	public Result<Boolean> deleteUumsUserInfoById(long id){
		try {
		    ...
		    if(uumsUserInfoDO == null){
				...
				return new Result(ResultCode.ILLEGAL_PARAMS, "id 为 "+id+" 的用户不存在");
		    }
		    ...
		    if(uumsUserInfoDO.getOperatorName().equals("admin")){
				...
				return new Result(ResultCode.ILLEGAL_PARAMS, "帐号  "+uumsUserInfoDO.getOperatorName()+" 的用户不能删除");
		    }
	...
	public Result<Boolean> updateUumsUserInfoById(UumsUserInfoModel userInfoModel) {
		try {
			...
			if(infoDO == null){
				...
				return new Result(ResultCode.ILLEGAL_PARAMS, "admin 帐号允许修改");
			...
			if(infoMobileDO != null && userInfoModel.getId() != infoMobileDO.getId()){
				...
				return new Result(ResultCode.DUPLICATE_DATA, "手机号 = "+userInfoModel.getMobile()+" 的帐号已存在");
			}
			...
			if(infoOperatorNameDO != null && userInfoModel.getId() != infoOperatorNameDO.getId()){
				...
				return new Result(ResultCode.DUPLICATE_DATA, "帐号  "+userInfoModel.getOperatorName()+" 已存在");
			}
	...
	public Result<UumsUserInfoModel> queryUumsUserInfoById(long id) {
		try {
			...
			if(userInfoDO == null){
				...
				return new Result(ResultCode.ILLEGAL_PARAMS, "用户ID "+id+" 不存在");
			}
			return new Result<UumsUserInfoModel>(UumsUserInfoConvertor.convertor2Model(userInfoDO));
		} catch (Exception ex) {
			LOGGER.error(ex.getMessage(), ex);
			return new Result(ResultCode.SERVER_EXCEPTION, ex.getMessage());
		}
	...
	public Result<Boolean> insertUumsUserPermissionRelation(final Long[] permissionIds, final Long userId) {
		return transactionTemplate.execute(new TransactionCallback<Result<Boolean>>() {

			public Result<Boolean> doInTransaction(TransactionStatus status) {
				try {
	...
	public Result<Boolean> deleteUumsUserPermissionRelation(final Long[] permissionIds, final Long userId) {
		return transactionTemplate.execute(new TransactionCallback<Result<Boolean>>() {
			public Result<Boolean> doInTransaction(TransactionStatus status) {
				try {
					for (Long permissionId : permissionIds) {
